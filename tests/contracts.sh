...
grep -Fq 'gtk_paned_pack1(split, controls_scroll, 1, 1)' "$ROOT_DIR/rust/src/gui.rs" || fail 'GUI sin área desplazable visible'
...
grep -Fq 'gtk_scrolled_window_set_policy(controls_scroll, 0, 1)' "$ROOT_DIR/rust/src/gui.rs" || fail 'GUI GTK sin scroll automático para ventanas pequeñas'
