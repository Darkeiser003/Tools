...
grep -Fq 'LTOOLS_GUI_TREE_CANCEL_REQUESTED_MARKER' "$ROOT_DIR/tests/linux/storage-map-gui-e2e.sh" || fail 'E2E del mapa no valida la petición de cancelación'
grep -Fq 'LTOOLS_GUI_TREE_CANCELLED_MARKER' "$ROOT_DIR/tests/linux/storage-map-gui-e2e.sh" || fail 'E2E del mapa no valida el estado final cancelado'
grep -Fq 'STORAGE_TREE_CANCEL_REQUESTED' "$ROOT_DIR/tests/linux/storage-map-gui-e2e.sh" || fail 'E2E del mapa no pulsa/observa la cancelación real'
grep -Fq 'STORAGE_TREE_CANCELLED' "$ROOT_DIR/tests/linux/storage-map-gui-e2e.sh" || fail 'E2E del mapa no comprueba que la cancelación termina limpiamente'
...
grep -Fq -- '--test-existing-cli PATH' "$ROOT_DIR/scripts/build.sh" || fail 'builder Linux no ofrece pruebas sobre perfiles CLI existentes'
