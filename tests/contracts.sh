...
grep -Fq -- "'--features', 'cli'" "$ROOT_DIR/windows/build.ps1" || fail 'builder Windows no compila el perfil CLI separado'
...
grep -Fq 'Compress-Archive' "$ROOT_DIR/windows/build.ps1" || fail 'builder Windows no genera el ZIP portable'
