...
grep -Fq "\$clippyCheck = @('clippy', '--locked', '--manifest-path', \$CargoManifest, '--all-targets', '--target', \$Target" "$ROOT_DIR/scripts/build.ps1" || fail 'builder Windows no pasa Clippy por todos los targets nativos con lock estricto'
grep -Fq "\$formatCheck = @('fmt', '--manifest-path', \$CargoManifest, '--', '--check')" "$ROOT_DIR/scripts/build.ps1" || fail 'builder Windows no valida el formato Rust'
...
grep -Fq 'cp -a -- "$ROOT_DIR/appimage/run-ltools.sh" "$RUNNER_ARTIFACT"' "$ROOT_DIR/scripts/build.sh" || fail 'la release no genera su lanzador desde el recurso versionado del AppImage'
