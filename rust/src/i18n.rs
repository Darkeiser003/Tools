...
pub fn storage_map_text(key: &str) -> &'static str {
    const MAP_TEXT: &[(&str, [&str; 15])] = &[
        ...
        (
            "explain_admin_home",
            ["مجلد مسؤول النظام؛ قد يحتوي إعدادات وبيانات حساسة، ولا يُعد مجلد مستخدم عاديًا.", "Administratorenverzeichnis; kann sensible Einstellungen und Daten enthalten und ist kein normales Benutzerverzeichnis.", "Administrator home; may contain sensitive settings and data and is not a normal user folder.", "Directorio personal del administrador; puede contener datos sensibles y no es una carpeta de usuario normal.", "Dossier personnel de l’administrateur ; peut contenir des données sensibles et n’est pas un dossier utilisateur ordinaire.", "व्यवस्थापक का होम; संवेदनशील सेटिंग और डेटा हो सकते हैं, यह सामान्य उपयोगकर्ता फ़ोल्डर नहीं है।", "Home dell’amministratore; può contenere dati sensibili e non è una cartella utente ordinaria.", "管理者のホームです。機密設定やデータを含む可能性があり、通常のユーザーフォルダーではありません。", "관리자 홈입니다. 민감한 설정과 데이터가 있을 수 있으며 일반 사용자 폴더가 아닙니다.", "Katalog domowy administratora; może zawierać wrażliwe dane i nie jest zwykłym folderem użytkownika.", "Pasta pessoal do administrador; pode conter dados sensíveis e não é uma pasta de utilizador normal.", "Dosarul personal al administratorului; poate conține date sensibile și nu este un dosar obișnuit de utilizator.", "Домашний каталог администратора; может содержать чувствительные настройки и данные и не является обычным каталогом пользователя.", "Домашній каталог адміністратора; може містити чутливі налаштування й дані та не є звичайним каталогом користувача.", "管理员主目录；可能包含敏感设置和数据，不是普通用户文件夹。"],
        ),
        (
            "explain_system_commands",
            ["أوامر النظام الأساسية؛ غالبًا تكون روابط إلى /usr، ولا تُنقل أو تُحذف يدويًا.", "Grundlegende Systembefehle; oft Links nach /usr, nicht manuell verschieben oder löschen.", "Core system commands; often links into /usr, so do not move or delete them manually.", "Comandos esenciales del sistema; a menudo son enlaces a /usr, no mover ni borrar manualmente.", "Commandes système essentielles ; souvent des liens vers /usr, ne pas déplacer ni supprimer manuellement.", "मुख्य सिस्टम कमांड; अक्सर /usr की लिंक होती हैं, इन्हें मैन्युअल रूप से न हटाएँ।", "Comandi di sistema fondamentali; spesso collegamenti a /usr, non spostarli né eliminarli manualmente.", "基本システムコマンドです。多くは /usr へのリンクなので、手動で移動・削除しないでください。", "핵심 시스템 명령입니다. 대개 /usr의 링크이므로 직접 이동하거나 삭제하지 마세요.", "Podstawowe polecenia systemu; często są dowiązaniami do /usr, nie przenoś ani nie usuwaj ich ręcznie.", "Comandos essenciais do sistema; muitas vezes são ligações para /usr, não mova nem elimine manualmente.", "Comenzi de bază ale sistemului; adesea sunt legături către /usr, nu le muta și nu le șterge manual.", "Основные системные команды; часто это ссылки в /usr, не перемещайте и не удаляйте вручную.", "Основні системні команди; часто це посилання на /usr, не переміщуйте й не видаляйте вручну.", "核心系统命令；通常是指向 /usr 的链接，请勿手动移动或删除。"],
        ),
        (
            "explain_system_libraries",
            ["مكتبات النظام وواجهات ABI؛ قد تكون روابط توافقية، ولا تُعدّل يدويًا.", "Systembibliotheken und ABI-Schnittstellen; können Kompatibilitätslinks sein, nicht manuell ändern.", "System libraries and ABI links; they may be compatibility links and should not be edited manually.", "Librerías del sistema y enlaces ABI; pueden ser enlaces de compatibilidad y no deben editarse manualmente.", "Bibliothèques système et liens ABI ; ils peuvent assurer la compatibilité et ne doivent pas être modifiés manuellement.", "सिस्टम लाइब्रेरी और ABI लिंक; ये संगतता लिंक हो सकते हैं, मैन्युअल रूप से न बदलें।", "Librerie di sistema e collegamenti ABI; possono essere link di compatibilità, non modificarli manualmente.", "システムライブラリと ABI リンクです。互換性用リンクの場合があるため、手動で変更しないでください。", "시스템 라이브러리와 ABI 링크입니다. 호환성 링크일 수 있으므로 직접 수정하지 마세요.", "Biblioteki systemowe i dowiązania ABI; mogą służyć zgodności, nie zmieniaj ich ręcznie.", "Bibliotecas do sistema e ligações ABI; podem ser ligações de compatibilidade, não altere manualmente.", "Biblioteci de sistem și legături ABI; pot fi legături de compatibilitate, nu le modifica manual.", "Системные библиотеки и ссылки ABI; могут быть ссылками совместимости, не изменяйте вручную.", "Системні бібліотеки та посилання ABI; можуть бути посиланнями сумісності, не змінюйте вручну.", "系统库和 ABI 链接；可能是兼容性链接，请勿手动修改。"],
        ),
        (
            "explain_optional_software",
            ["برامج اختيارية خاصة بالنظام أو بجهة خارجية؛ أدرها عبر الحزمة أو المثبّت الذي أنشأها.", "Optionale System- oder Drittsoftware; mit dem Paketmanager oder Installer verwalten, der sie erstellt hat.", "Optional system or third-party software; manage it with the package manager or installer that created it.", "Software opcional del sistema o de terceros; gestiónala con el gestor o instalador que la creó.", "Logiciels optionnels du système ou de tiers ; gérez-les avec le gestionnaire ou l’installeur qui les a créés.", "वैकल्पिक सिस्टम या तृतीय-पक्ष सॉफ़्टवेयर; उसे बनाने वाले पैकेज प्रबंधक या इंस्टॉलर से प्रबंधित करें।", "Software opzionale di sistema o di terze parti; gestiscilo con il gestore pacchetti o installer che l’ha creato.", "任意のシステムまたはサードパーティ製ソフトウェアです。作成元のパッケージ管理やインストーラーで管理してください。", "선택적 시스템 또는 타사 소프트웨어입니다. 만든 패키지 관리자나 설치 프로그램으로 관리하세요.", "Opcjonalne oprogramowanie systemowe lub zewnętrzne; zarządzaj nim przez menedżer pakietów lub instalator, który je utworzył.", "Software opcional do sistema ou de terceiros; faça a gestão com o gestor de pacotes ou instalador que o criou.", "Software opțional de sistem sau terță parte; gestionează-l cu managerul de pachete sau instalatorul care l-a creat.", "Дополнительное системное или стороннее ПО; управляйте им через создавший его пакетный менеджер или установщик.", "Додаткове системне або стороннє ПЗ; керуйте ним через менеджер пакунків або інсталятор, який його створив.", "可选的系统或第三方软件；请使用创建它的包管理器或安装程序管理。"],
        ),
        (
            "explain_mount_root",
            ["جذر نقاط التركيب؛ تُوصل فيه أنظمة ملفات وأقراص أخرى، فلا تحذف محتواه قبل معرفة مصدر التركيب.", "Wurzel für Einhängepunkte; hier werden andere Dateisysteme und Datenträger eingebunden, Quelle vor dem Löschen prüfen.", "Mount root; other filesystems and disks are attached here, so identify the mount source before changing contents.", "Raíz de montajes; aquí se conectan otros sistemas de archivos y discos, identifica el origen antes de modificar.", "Racine des montages ; d’autres systèmes de fichiers et disques y sont attachés, vérifiez la source avant toute modification.", "माउंट रूट; अन्य फ़ाइल सिस्टम और डिस्क यहाँ जुड़ते हैं, बदलने से पहले स्रोत पहचानें।", "Radice dei mount; qui vengono collegati altri filesystem e dischi, identifica l’origine prima di modificare.", "マウントポイントのルートです。別のファイルシステムやディスクが接続されるため、変更前に接続元を確認してください。", "마운트 루트입니다. 다른 파일 시스템과 디스크가 연결되므로 변경 전에 마운트 출처를 확인하세요.", "Katalog główny montowań; podłącza się tu inne systemy plików i dyski, przed zmianą sprawdź źródło.", "Raiz dos pontos de montagem; outros sistemas de ficheiros e discos são ligados aqui, confirme a origem antes de alterar.", "Rădăcina punctelor de montare; aici sunt atașate alte sisteme de fișiere și discuri, verifică sursa înainte de modificare.", "Корень точек монтирования; сюда подключаются другие файловые системы и диски, перед изменением выясните источник.", "Корінь точок монтування; тут під’єднуються інші файлові системи й диски, перед зміною визначте джерело.", "挂载根目录；其他文件系统和磁盘会连接到此处，修改前请确认挂载来源。"],
        ),
        (
            "explain_removable_media",
            ["وسائط قابلة للإزالة؛ قد تتغير عند توصيل جهاز، افصلها بأمان ولا تحذف دون تحديد القرص.", "Wechselmedien; ändern sich beim Anschließen von Geräten. Sicher aushängen und Datenträger vor dem Löschen prüfen.", "Removable media; contents change when devices are connected. Safely unmount and identify the disk before deleting.", "Medios extraíbles; cambian al conectar dispositivos. Desmonta con seguridad e identifica el disco antes de borrar.", "Supports amovibles ; leur contenu change quand un appareil est connecté. Démontez-les proprement et identifiez le disque avant suppression.", "हटाने योग्य मीडिया; डिवाइस जोड़ने पर सामग्री बदलती है। सुरक्षित रूप से अनमाउंट करें और हटाने से पहले डिस्क पहचानें।", "Supporti rimovibili; il contenuto cambia collegando dispositivi. Smonta in sicurezza e identifica il disco prima di eliminare.", "リムーバブルメディアです。機器接続で内容が変わるため、安全にアンマウントし、削除前にディスクを確認してください。", "이동식 미디어입니다. 장치 연결에 따라 내용이 바뀌므로 안전하게 마운트 해제하고 삭제 전 디스크를 확인하세요.", "Nośniki wymienne; zawartość zmienia się po podłączeniu urządzeń. Odmontuj bezpiecznie i sprawdź dysk przed usunięciem.", "Suportes amovíveis; o conteúdo muda quando os dispositivos são ligados. Desmonte em segurança e identifique o disco antes de eliminar.", "Medii detașabile; conținutul se schimbă la conectarea dispozitivelor. Demontează în siguranță și identifică discul înainte de ștergere.", "Съёмные носители; содержимое меняется при подключении устройств. Безопасно отключайте их и проверяйте диск перед удалением.", "Знімні носії; вміст змінюється після під’єднання пристроїв. Безпечно демонтуйте їх і перевірте диск перед видаленням.", "可移动介质；连接设备时内容会变化，请安全卸载并在删除前确认磁盘。"],
        ),
        (
            "explain_service_data",
            ["بيانات الخدمات التي تقدمها هذه الآلة؛ لا تنقلها أو تحذفها دون معرفة الخدمة المالكة.", "Daten lokaler Dienste; nicht verschieben oder löschen, ohne den zuständigen Dienst zu kennen.", "Data served by this machine; do not move or delete it without identifying the owning service.", "Datos servidos por esta máquina; no mover ni borrar sin identificar el servicio propietario.", "Données servies par cette machine ; ne pas déplacer ni supprimer sans identifier le service propriétaire.", "इस मशीन द्वारा दी जाने वाली सेवा का डेटा; स्वामी सेवा जाने बिना न हटाएँ या स्थानांतरित करें।", "Dati serviti da questa macchina; non spostarli né eliminarli senza identificare il servizio proprietario.", "このマシンが提供するサービスデータです。所有するサービスを確認せずに移動・削除しないでください。", "이 컴퓨터가 제공하는 서비스 데이터입니다. 소유 서비스를 확인하지 않고 이동하거나 삭제하지 마세요.", "Dane usług udostępnianych przez tę maszynę; nie przenoś ani nie usuwaj bez ustalenia usługi właściciela.", "Dados dos serviços fornecidos por esta máquina; não mova nem elimine sem identificar o serviço proprietário.", "Date deservite de această mașină; nu le muta și nu le șterge fără să identifici serviciul proprietar.", "Данные служб, предоставляемых этой машиной; не перемещайте и не удаляйте без выяснения службы-владельца.", "Дані служб, які надає ця машина; не переміщуйте й не видаляйте, не визначивши службу-власника.", "此计算机提供的服务数据；确认所属服务前请勿移动或删除。"],
        ),
        (
            "explain_recovery_data",
            ["بيانات استرداد نظام الملفات؛ لا تحذفها إلا بعد التحقق من أن القرص ليس قيد الإصلاح.", "Dateien zur Dateisystemwiederherstellung; nur löschen, wenn keine Reparatur mehr läuft.", "Filesystem recovery data; delete only after confirming the disk is not undergoing repair.", "Datos de recuperación del sistema de archivos; borrar solo tras confirmar que el disco no está en reparación.", "Données de récupération du système de fichiers ; supprimer seulement après vérification qu’aucune réparation n’est en cours.", "फ़ाइल सिस्टम रिकवरी डेटा; डिस्क की मरम्मत न चल रही हो तभी हटाएँ।", "Dati di recupero del filesystem; elimina solo dopo aver verificato che il disco non sia in riparazione.", "ファイルシステムの復旧データです。ディスクの修復中でないことを確認してから削除してください。", "파일 시스템 복구 데이터입니다. 디스크 복구가 진행 중이 아닌지 확인한 후 삭제하세요.", "Dane odzyskiwania systemu plików; usuwaj dopiero po upewnieniu się, że dysk nie jest naprawiany.", "Dados de recuperação do sistema de ficheiros; elimine apenas depois de confirmar que o disco não está a ser reparado.", "Date de recuperare a sistemului de fișiere; șterge-le doar după ce verifici că discul nu este reparat.", "Данные восстановления файловой системы; удаляйте только убедившись, что диск не ремонтируется.", "Дані відновлення файлової системи; видаляйте лише після перевірки, що диск не ремонтується.", "文件系统恢复数据；确认磁盘未处于修复过程中后再删除。"],
        ),
        (
